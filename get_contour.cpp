...
void getcontour(char* sketchsource)
{
    // char* sourcefile = "sketches/640400.png";
    cout<<"Checking at location "<<sketchsource<<endl;
    CImg<unsigned char> src(sketchsource);
    int done,dir;
    ...
    /*---------------------------------- moores contour --------------------------------*/
    cout<<"Fetching the contour points.."<<endl;
  	dir=2;
  	done = 2;
  	int initi=r,initj=c;
  	vector<int> ni,nj;
    ...
    cout<<"Writing .poly file for triangle"<<endl;
  	vector <int> :: iterator ciiter,cjiter;
  	ofstream outdata;
  	outdata.open("triangle/contour.poly");
  	if(!outdata)
  	{
  		cout<<"Error opening file"<<endl;exit(0);
  	}
